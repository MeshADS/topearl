...
@section('jumbotron_breadcrumb')
	<ul class="breadcrumb">
	    ...
	    <li><a href="{{ URL::to('admin/headers') }}" class="active">Headers</a>
	    </li>
	</ul>
@stop
@section('jumbotron')
	...
	<div class="content-fluid">
		...
		<div class="col-md-12">
			...
			{{-- Begin Modals --}}
					...
					<div class="modal fade slide-down disable-scroll" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createModalLabel" aria-hidden="true">
						...
						<div class="modal-dialog">
							...
							<div class="modal-content">
								{{ Form::open(["url"=>"admin/headers", "role"=>"form", "enctype"=>"multipart/form-data"]) }}
									...
									<div class="modal-body">
										...
										<div class="form-group form-group-default m-t-10">
											...
											<small>Dimension: 480 x 640</small>
										</div>
										...
										<div class="form-group form-group-default m-t-10">
											...
											{{ Form::text("title", "", ["class"=>"form-control", "placeholder"=>"E.g. Home, About, Contact"]) }}
										</div>
					...
					@foreach($list as $item)
						<div class="modal fade fill-in" id="previewModal{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="previewModalLabel{{ $item->id }}" aria-hidden="true">
							...
							<div class="modal-dialog">
								...
								<div class="modal-content">
									{{ Form::open(["url"=>"admin/headers/".$item->id, "role"=>"form", "method"=>"put", "enctype"=>"multipart/form-data"]) }}
										<div class="modal-header">
											{{-- Empty header --}}
										</div>
										...
										<div class="modal-body">
											...
											<ul class="list-unstyled">
												...
												<li>
													<h4><strong>{{ $item->page->name }}</strong></h4>
												</li>
				...
				{{-- Begin Mobile Preview Header Modal --}}
					@foreach($list as $item)
						@if(!empty($item->mobile_image))
							<div class="modal fade fill-in" id="mobilePreviewModal{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="previewModalLabel{{ $item->id }}" aria-hidden="true">
								<button class="close" data-dismiss="modal" aria-label="close">
									<span aria-hidden="true"><i class="fa fa-times"></i></span>
								</button>
									...
									<div class="modal-content">
										...
										<div class="modal-body">
											...
											<ul class="list-unstyled">
												...
												<li>
													<img src="{{ URL::to($item->mobile_image) }}" style="max-width:100%;">
												</li>
							...
							@endif
					@endforeach
				{{-- End Mobile Preview Header Modal --}}

				{{-- Begin Edit Header Modal --}}
					@foreach($list as $item)
									...
									{{ Form::open(["url"=>"admin/headers/".$item->id, "role"=>"form", "method"=>"put", "enctype"=>"multipart/form-data"]) }}
										...
										<div class="modal-body">
											...
											<div class="form-group form-group-default m-t-10">
												...
												<select name="page_id" class="form-control" id="page_id">
													@foreach($pages as $k => $v)
														<option value="{{ $k }}" {{ ($item->page_id == $k) ? ' selected ' : '' }}>{{ $v }}</option>
													@endforeach
												</select> 
											</div>
											...
											<div class="form-group form-group-default m-t-10">
												...
												<input class="form-control" placeholder="E.g. Home, About, Contact" name="title" type="text" value="{{ $item->title }}" id="title">
											</div>
					...
					@foreach($list as $item)
						<div class="modal fade slide-down disable-scroll" id="deleteModal{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel{{ $item->id }}" aria-hidden="true">
							...
							<div class="modal-dialog modal-sm">
								...
								<div class="modal-content">
									{{ Form::open(["url"=>"admin/headers/".$item->id, "role"=>"form", "method"=>"delete"]) }}
										<div class="modal-header">
											<button class="close" data-dismiss="modal" aria-label="close">
												<span aria-hidden="true"><i class="fa fa-times"></i></span>
											</button>
				...
				{{-- Begin Bulk Delete Header Modal --}}
					<div class="modal fade slide-down disable-scroll" id="bulkDeleteModal" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel" aria-hidden="true">
						...
						<div class="modal-dialog modal-sm">
							...
							<div class="modal-content">
								{{ Form::open(["url"=>"admin/headers/bulk", "role"=>"form", "method"=>"delete"]) }}
									<div class="modal-header">
										<button class="close" data-dismiss="modal" aria-label="close">
											<span aria-hidden="true"><i class="fa fa-times"></i></span>
										</button>
								...
								<div class="panel panel-default">
									<div class="panel-heading">
										<h4 class="panel-title">
											...
											<a href="#" data-toggle="modal" data-target="#previewModal{{ $item->id }}">
											{{ (isset($item->page->name)) ? $item->page->name : $item->page_id }}
											</a>
										</h4>
										<div class="panel-controls">
											<ul>
												@if(!empty($item->mobile_image))
													<li>
														<a href="#" 
															class="portlet-collapse" 
															data-toggle="modal" 
															data-target="#mobilePreviewModal{{ $item->id }}"
