...
@section('content')
	...
	<div class="content-fluid">
		...
		<div class="col-md-12">
			...
			{{ Form::open(["url"=>"admin/admission", "enctype"=>"multipart/form-data"]) }}
				<div class="container-fluid">
					...
					<div class="col-md-8">
						...
						<div class="form-group">
							{{ Form::textarea("description", "", ["class"=>"form-control", "id"=>"description", "rows"=>"6"]) }}
					...
					<div class="col-md-4">
					        	...
					        	@foreach($contacts as $key => $group)
						            <optgroup label="{{ ucwords(str_replace('-', ' ', $key)) }}">
						            	@foreach($group as $contact)
						                	<option value="{{ $contact->id }}">{{ $contact->data }}</option>
						                @endforeach
						            </optgroup>
					            @endforeach
					        </select>
						...
						<div class="input-group date myDatepicker m-b-10">
						    {{ Form::text("open_date", "", ["class"=>"form-control", "placeholder"=>"Open date"]) }}
						    <span class="input-group-addon"><i class="fa fa-calendar"></i>
						    </span>
						</div>

						<div class="input-group date myDatepicker m-b-10">
						    {{ Form::text("close_date", "", ["class"=>"form-control", "placeholder"=>"Close date"]) }}
						    <span class="input-group-addon"><i class="fa fa-calendar"></i>
						    </span>
						</div>
						...
						<div class="form-group">
							...
							<button type="submit" class="btn btn-warning btn-lg btn-block">Create</button>
