...
@section('jumbotron_breadcrumb')
	<ul class="breadcrumb">
	    ...
	    <li><a href="{{ Request::url() }}" class="active">Forms</a>
	    </li>
	</ul>
@stop
@section('jumbotron')
					...
					<div class="modal fade slide-down disable-scroll" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createModalLabel" aria-hidden="true">
						...
						<div class="modal-dialog">
							...
							<div class="modal-content">
								{{ Form::open(["url"=>"admin/forms", "role"=>"form"]) }}
									...
									<div class="modal-body">
										...
										<div class="form-group form-group-default m-t-10">
											...
											{{ Form::text("name", "", ["class"=>"form-control", "placeholder"=>"Enter form name here.."]) }}
										</div>
										...
										<div class="form-group form-group-default m-t-10">
											...
											<input type="checkbox" name="notify" value="1" class="switchery" data-color="#f8d053"><br>
											<small><i>Check this field if you would like to recieve an email notification everytime this form is submitted.</i></small>
					...
					@foreach($list as $item)
						<div class="modal fade slide-down disable-scroll" id="editModal{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{ $item->id }}" aria-hidden="true">
							...
							<div class="modal-dialog">
								...
								<div class="modal-content">
									{{ Form::open(["url"=>"admin/forms/".$item->id, "role"=>"form", "method"=>"put"]) }}
										<div class="modal-header">
										<button class="close" data-dismiss="modal" aria-label="close">
											<span aria-hidden="true"><i class="fa fa-times"></i></span>
										</button>
									...
									<div class="modal-body">
										...
										<div class="form-group form-group-default m-t-10">
											{{ Form::label("name", "Name") }}
											<input class="form-control" placeholder="Enter form name here.." name="name" type="text" value="{{ $item->name }}" id="name">
										</div>

										<div class="form-group form-group-default m-t-10">
											{{ Form::label("publish", "Publish?") }}
											<input type="checkbox" name="publish" value="1" class="switchery" data-color="#f8d053" {{ ($item->publish == 1) ? 'checked' : '' }}>
										</div>

										<div class="form-group form-group-default m-t-10">
											{{ Form::label("notify", "Notify?") }}
											<input type="checkbox" name="notify" value="1" class="switchery" data-color="#f8d053" {{ ($item->notify == 1) ? 'checked' : '' }}><br>
											<small><i>Check this field if you would like to recieve an email notification everytime this form is submitted.</i></small>
										</div>
					...
					@foreach($list as $item)
						<div class="modal fade slide-down disable-scroll" id="deleteModal{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel{{ $item->id }}" aria-hidden="true">
							...
							<div class="modal-dialog">
								...
								<div class="modal-content">
									{{ Form::open(["url"=>"admin/forms/".$item->id, "role"=>"form", "method"=>"delete"]) }}
										<div class="modal-header">
											<button class="close" data-dismiss="modal" aria-label="close">
												<span aria-hidden="true"><i class="fa fa-times"></i></span>
											</button>
				...
				{{-- Begin Bulk Delete Form Modal --}}
					<div class="modal fade slide-down disable-scroll" id="bulkDeleteModal" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel" aria-hidden="true">
						...
						<div class="modal-dialog">
							...
							<div class="modal-content">
								{{ Form::open(["url"=>"admin/forms/bulk", "role"=>"form", "method"=>"delete"]) }}
									<div class="modal-header">
										<button class="close" data-dismiss="modal" aria-label="close">
											<span aria-hidden="true"><i class="fa fa-times"></i></span>
										</button>
				    ...
				    <tbody>
				    	@foreach($list as $item)
					        <tr>
					            ...
					            <td>{{ $item->slug }}</td>
					            <td>
					            	<span class="label label-{{ ($item->publish == 1) ? 'success' : 'default'}}">
					            		{{ ($item->publish == 1) ? 'Published' : 'Unpublished' }}
					            	</span>
					            </td>
					            <td>
					            	<span class="label label-{{ ($item->notify == 1) ? 'success' : 'default'}}">
					            		{{ ($item->notify == 1) ? 'Enabled' : 'Disabled' }}
					            	</span>
					            </td>
					            <td>
					            	<strong>{{ count($item->elements) }}</strong>
					            </td>
					            <td>
					            	<strong>{{ count($item->submitions) }}</strong>
					            </td>
					            <td>
					            	<div class="btn-group dropdown-default">
									    ...
									    <ul class="dropdown-menu ">
									        ...
									        <li><a href="{{ URL::to('admin/forms/'.$item->id.'/elements') }}">Elements</a>
									        </li>
									        <li><a href="{{ URL::to('admin/forms/'.$item->id.'/submitions') }}">Submitions</a>
									        </li>
									    </ul>
									</div>
					            </td>
