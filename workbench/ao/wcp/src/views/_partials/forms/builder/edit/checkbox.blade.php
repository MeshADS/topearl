<div class="myFormWizard">
    ...
    <ul class="nav nav-tabs nav-tabs-linetriangle nav-tabs-separator">
        <li class="active">
            <a data-toggle="tab" href="#c{{$item->id}}tab1"><i class="fa fa-info tab-icon"></i> <span>Basic Info</span></a>
        </li>
        <li class="">
            <a data-toggle="tab" href="#c{{$item->id}}tab2"><i class="fa  fa-list tab-icon"></i> <span>List</span></a>
        </li>
        <li class="">
            <a data-toggle="tab" href="#c{{$item->id}}tab3"><i class="fa fa-lock tab-icon"></i> <span>Rules</span></a>
        </li>
        <li class="">
            <a data-toggle="tab" href="#c{{$item->id}}tab4"><i class="pg-grid tab-icon"></i> <span>Layout</span></a>
        </li>
    </ul>
    <!-- Tab panes -->
    <div class="tab-content" style="padding-left:0px; padding-right:0px;">
        <div class="tab-pane active slide" id="c{{$item->id}}tab1">
        ...
        <div class="tab-pane slide" id="c{{$item->id}}tab2">
            <div class="panel panel-transparent">
				<div class="panel-body listValues" id="listvalues{{ $item->id }}" style="padding:0px 0px;">
                    @foreach($item->listValues as $lv)
    					<div class="row p-r-35 m-t-10 listvalues" id="echeckbox{{ $item->id }}-{{ $lv->id }}">
                            ...
                            <a href="javascript:;" title="Delete!" class="new-element-dl-btn remove-olistvalue" data-target="#echeckbox{{ $item->id }}-{{ $lv->id }}">
                                <i class="fa fa-times"></i>
                            </a>
                            <div class="delMessage p-l-15 p-r-15 p-t-20">
                                This option will be deleted after you save changes.
                                <a href="#" title="Cancel this action" class="cancel-remove-olistvalue" data-target="#echeckbox{{ $item->id }}-{{ $lv->id }}">[Cancel]</a>
                            </div>
                        </div>
                    @endforeach
				</div>
				...
				<div class="panel-body" style="padding-left:0px; padding-right:0px;">
					<p class="text-center" id="checkboxModalHelper{{$item->id}}" style="{{ (count($item->listValues) > 0) ? 'display:none;' :'' }}">
						<small>
							Click the plus sign to add a new checkbox
						</small>
					</p>
					<button type="button" class="btn btn-block btn-complete addListvalue" data-id="{{ $item->id }}" data-helper="#checkboxModalHelper{{$item->id}}" data-target="#listvalues{{ $item->id }}" title="Add a checkbox">
						<i class="fa fa-plus-circle"></i>
					</button>
				</div>
			</div>
        </div>
        <div class="tab-pane slide" id="c{{$item->id}}tab3">
             <div class="panel panel-transparent">
        ...
        <div class="tab-pane slide" id="c{{$item->id}}tab4">
            <div class="panel panel-transparent">
                <div class="panel-body" style="padding-left:0px; padding-right:0px;">
                    <p>
                      <small>
